"""
Procedurally generate scenes/mexican_hat_drum_modes.xml: a 10×10 mass-spring
lattice (drum-head analog) with only vertical motion.  Interior nodes slide on
z; edges are welded to a square frame.  Neighbors are coupled by stiff spatial
tendons.  Initial displacements match the (1,1) mode
    z_ij = A · sin(π i / 9) · sin(π j / 9),  A = 0.02 m.
"""
import math
import os
    ...
    for j in range(N):
        ...
        if edge:
            bodies.append(
                ...
                f'size="{R}" mass="{M}" material="membrane" '
            ...
            bodies.append(
                ...
                f'material="membrane" contype="0" conaffinity="0"/>\n'
    ...
    for j in range(N):
        for di, dj in ((1, 0), (0, 1)):
            ...
            tendons.append(
                ...
                f'stiffness="{K_LINK}" damping="{D_LINK}" '
                f'springlength="{rest:.5f}" width="0.0015" '
                f'rgba="0.45 0.42 0.40 0.55" limited="false">\n'
                f'      <site site="s{i}_{j}"/>\n'
...
xml = f"""<mujoco model="mexican_hat_drum_modes">
  <!-- 10×10 vertical mass-spring lattice; (1,1) Mexican-hat mode initial shape. -->
  ...
  <asset>
    ...
    <material name="membrane" rgba="0.30 0.50 0.78 1" specular="0.4" shininess="0.55"/>
  </asset>
  ...
  <worldbody>
    ...
    <!-- Square frame under the membrane (visual anchor for fixed edges). -->
...
out_path = os.path.join(os.path.dirname(__file__), "mexican_hat_drum_modes.xml")
with open(out_path, "w") as f:
    f.write(xml)

n_interior = len(qpos_parts)
print(f"Wrote {out_path}  ({N}×{N} nodes, {n_interior} slide DOFs, "
      f"{len(tendons)} link tendons)")
print(f'Suggested --init-qpos: "{init_qpos}"')
print("Render 3.0 s:")
print(f'  python3 render.py --scene scenes/mexican_hat_drum_modes.xml '
      f'--out out/new_scenes/mexican_hat_drum_modes.mp4 --duration 3.0 '
